...
When used as a value in a key/value pair of a [type_map](./type_map.md), it is equivalent to the key being absent from that entity. If an empty value that maintains the key's presence is desired, use [no_value_descriptor](./no_value_descriptor.md) instead.
